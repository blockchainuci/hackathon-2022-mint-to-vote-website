...
contract Proposal {
    struct Option {
        ...
        uint256 voteCount; //cumulative votes for said option
    }
    ...
    constructor(
        ...
        for (uint256 i = 0; i < optionNames.length; i++) {
            options.push(Option({optionName: optionNames[i], voteCount: 0}));
        }
    }
    ...
    function winningOption() public view returns (uint256 winningOption_) {
        uint256 winningOptionCount = 0;
